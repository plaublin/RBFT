...
#   -the malicious node is node 0 (assuming f=1)
#   -clients send a proportion g of invalid requests to the correct nodes
#   -malicious nodes flood the correct nodes
#   -malicious replicas of the backup instances flood the correct ones
#   -malicious replicas of the master instance DO participate in the protocol
...
OUTDIR="rrbft_${NB_CLIENTS}clients_${REQ_SIZE}B_perc_faulty_clients_${PERCENTAGE_C}_attack2_openloop_$(date +%F_%H_%M)"
