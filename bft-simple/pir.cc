/*
 * This launches a Protocol Instance Replica.
 */
...
int PIR_MAIN(int argc, char **argv)
{
  ...
  int opt;
  while ((opt = getopt(argc, argv, "c:p:d:z")) != EOF)
  {
    switch (opt)
    {
    case 'c':
    ...
    case 'd':
      byz_pre_prepare_delay = atoi(optarg);
      break;
    ...
    default:
      fprintf(stderr, "%s -c config_file -p config_priv_file -d pre_prepare_delay -z", argv[0]);
      exit(-1);
    }
  }
  ...
  if (byz_pre_prepare_delay > 0)
  {
    fprintf(
        stderr,
        "I am faulty and I will add a delay of %ims before sending every pre-prepare\n",
...
//  sigaction(SIGTERM, &act, NULL);
  ...
  fprintf(stderr, "Going to initialize the PIR with files %s and %s\n", config, config_priv);
  ...
  fprintf(stderr, "PIR has been initialized!\n");

  // 2) Loop executing requests.
  Byz_replica_run();
