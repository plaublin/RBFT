...
for PORT in `grep $(hostname) $CONFIG | awk '{print $3}'` ; do
  if [ $i -lt $NB_CORRECT_CLIENTS ] ; then
     ...
     if [ -z $DEBUG ]; then
       taskset -c $core ./bft_client -c $CONFIG -C $CONFIG_PRIV -p $PORT -g 0 -m $MANAGER_IP 2>&1  &
     elif [ "$DEBUG" = "GDB" ]; then
        ########### GDB ##########
        ...
        taskset -c $core gdb -x /tmp/client_${i}.gdb ./bft_client &> /tmp/client_${i}.log &
     else
        echo "Unknown $DEBUG"
     fi
     ...
     if [ $j -lt $NB_FAULTY_CLIENTS ] ; then
       ...
       taskset -c $core ./bft_client -c $CONFIG -C $CONFIG_PRIV -p $PORT -g 1 -m $MANAGER_IP 2>&1  &
       j=$(($j+1))
       core=$((($core+1)%$NB_CORES))
     else
       if [ $NB_FAULTY_CLIENTS -eq -1 ]; then     
          ...
          taskset -c $core ./bft_client -c $CONFIG -C $CONFIG_PRIV -p $PORT -g 0 -m $MANAGER_IP -f 2>&1  &
       fi
       break
     fi
  fi
