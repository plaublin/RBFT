...
if [ $# -eq 5 ] ; then
    ...
    echo "attack = NODE_FLOODING, args = you need to put an arbitrary value (arg not used)"
...
if [ $ATTACK == "NODE_FLOODING" ]; then
  ./sedinplace.sh DYNAMIC_WORKLOAD_700_PROTOCOLS_NODE_FLOODING
else
...
for replica in $VERIFIERS; do
    ...
    if [ $ATTACK == "NODE_FLOODING" ]; then
      if [ "$machine" == "${FAULTY_NODE}" ] || ( [ $NB_FAULTS -eq 2 ] && ["$machine" == "${FAULTY_NODE2}" ] ); then
          VERIFIER_PIRS_ARGS="NODE_FLOODING"
      else
          VERIFIER_PIRS_ARGS=""
      fi
    fi
...
do
   ...
   ssh -n $machine "cd $BASE_DIR; nohup ./launch_clients_dynamic_workload_openloop.sh $manager_ip $nbc $PROPORTION_GOOD_REQUESTS &> launch_clients_${machine}.log &"
...
if [ $ATTACK == "PP_DELAY" ];then
   OUTDIR="rrbft_${REQ_SIZE}B_${NB_CLIENTS}clients_dynamic_workload_attack_pp_delay_${PP_DELAY}_openloop_$(date +%F_%H_%M)"

elif [ $ATTACK == "FAULTY_CLIENT_SENDS_VALID_REQ_TO_ONE_NODE" ]; then
   OUTDIR="rrbft_${REQ_SIZE}B_${NB_CLIENTS}clients_dynamic_workload_attack_proportion_requests_sent_to_all_nodes_${PROPORTION_GOOD_REQUESTS}_openloop_$(date +%F_%H_%M)"

elif [ $ATTACK == "NODE_FLOODING" ]; then
   OUTDIR="rrbft_${REQ_SIZE}B_${NB_CLIENTS}clients_dynamic_workload_node_flooding_openloop_$(date +%F_%H_%M)"

else
   OUTDIR="rrbft_${REQ_SIZE}B_${NB_CLIENTS}clients_dynamic_workload_openloop_$(date +%F_%H_%M)"
fi
