...
struct Reply_rep : public Message_rep {
  ...
  int client;            // id of the client for which this reply is
...
public:
  ...
  // Effects: Set the view of the message to v
  ...
  int cid() const;
  // Effects: return the client id for which this reply is

  void set_cid(int c);
  // Effects: set the client id of this reply
...
inline void Reply::set_view(View v)
{
  rep().v = v;
}
...
inline int Reply::cid() const { return rep().client; }

inline void Reply::set_cid(int c) { rep().client = c; }
