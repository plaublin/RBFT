...
void Request::authenticate_for_forwarding(void)
{
  int old_size = sizeof(Request_rep) + rep().command_size;

  node->gen_auth_out(contents(), sizeof(Request_rep), contents() + old_size);

  /*
  fprintf(stderr, "[authenticate_for_forwarding] Request of cid %i, id %i and size %i, Content is:\n", client_id(), id(), size());
...
bool Request::verify_by_forwarder(void)
{
  int old_size = sizeof(Request_rep) + rep().command_size;
  ...
  /*
  fprintf(stderr, "[verify_by_forwarder] Request of cid %i, id %i and size %i, Content is:\n", client_id(), id(), size());
  for (char* p = contents(); p < (contents() + size()); p++) {
      fprintf(stderr, " %i", (int)*p);
  }
  fprintf(stderr, "\n====================\n");
  */
  ...
  return node->verify_auth_in(id(), contents(), sizeof(Request_rep),
      contents() + old_size);
}
...
void Request::sign(int act_len)
{
  ...
  bzero(contents() + old_size, node->principal()->sig_size());
#else
  node->gen_signature(contents(), sizeof(Request_rep), contents() + old_size);
#endif
...
//FIXME: this method is called by Pre_prepare.cc:verify (line 214)
// and Wrapped_request.cc (from Verifier thread). So we should lock it
bool Request::verify_request()
{
...
#ifdef SIMULATE_SIGS
      //      sig_count++;
      pub_key->verify_request(_msg, sig);
...
Time Request::get_cost_of_verifying_sig()
{
  const int cid = node->n();
  Principal* p = node->i_to_p(cid);
  int nb_iter = 100;

  set_size(sizeof(Request_rep) + node->principal()->sig_size());

  Time start = currentTime();
  for (int i = 0; i < nb_iter; i++)
  {
    if (size() - (int)sizeof(Request_rep) >= p->sig_size())
    {
      p->verify_signature(contents(), sizeof(Request_rep), contents(), true);
    }
  }

  return (currentTime()-start)/nb_iter;
}

Time Request::get_cost_of_generating_mac()
{
  int nb_iter = 100;

  set_size(sizeof(Request_rep) + node->auth_size());

  Time start = currentTime();
  for (int i = 0; i < nb_iter; i++)
  {
    node->gen_auth_in(contents(), sizeof(Request_rep), contents() + sizeof(Request_rep), false);
  }

  // gen_auth_in generates a MAC authenticator: 1 MAC per replica.
  return (currentTime()-start)/(nb_iter*node->n());
}

Time Request::get_cost_of_verifying_mac()
{
  const int cid = node->n();
  int nb_iter = 100;
  ...
  for (int i = 0; i < nb_iter; i++)
  {
    node->verify_auth_in(cid, contents(), sizeof(Request_rep),
        contents() + sizeof(Request_rep));
  }
