...
public:
  ...
  State(Verifier *replica, char *memory, int num_bytes);
  ...
  bool restart(FILE* i, Verifier *rep, Seqno ls, Seqno le, bool corrupt); 
  ...
  void set_seqno_last_checkpoint(Seqno slc);
...
inline void State::set_seqno_last_checkpoint(Seqno slc) {
  lc = slc;
}
