...
public:
  ...
  void authenticate_for_forwarding(void);
  // Authenticate a Request that is going to be forwarded by the Forwarder thread

  bool verify_by_forwarder(void);
  // Authenticate a Request that is going to be forwarded by the Forwarder thread
  ...
  Time get_cost_of_verifying_sig();
  Time get_cost_of_generating_mac();
  Time get_cost_of_verifying_mac();

/**************SONIA: methods for the creation of a Certificate<Request>*****************/


  bool match(Request *r);
  // Effects: Returns true if the requests match.

  bool full() const;
  // Effects: Returns true iff "this" is a full request.
...
/*****************SONIA********************/
//For the time being we will use the field "replier" to store the id of the node which forwarded the request
inline int Request::id() const {return rep().replier; }

//For the time being full will return true
inline bool Request::full() const {return true;}

//For the time being match returns true if the digest of cid, rid and command are equal with those of r
inline bool Request::match(Request *r) {
fprintf(stderr,"[match:] rep.cid: %i, r.cid: %i, rep.rid: %qd, r.rid: %qd\n",rep().cid, r->rep().cid, rep().rid, r->rep().rid);
 return ((rep().cid == r->rep().cid && rep().rid == r->rep().rid));
}

//For the time being verify will return true
inline bool Request::verify() const {return true;}
/*****************************************/
