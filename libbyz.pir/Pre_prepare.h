...
public:
  ...
  Pre_prepare(View v, Seqno s, Req_list *rlist);
  // Effects: Creates a new signed Pre_prepare message with view
  // number "v", sequence number "s", the requests in "rlist" (up to a
  // maximum size) and appropriate non-deterministic choices.  It
  // removes the elements of "rlist" that are included in the message
  // from "rlist" and deletes them.
