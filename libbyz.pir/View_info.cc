...
View_info::View_info(int ident, View vi) :
  v(vi), id(ident), last_stable(0), oplog(max_out), last_views((View) 0,
      node->n()), last_vcs((View_change*) 0, node->n()), my_vacks(
      (View_change_ack*) 0, node->n()), vacks(node->n()), last_nvs(node->n())
{
...
void View_info::view_change(View vi, Seqno last_executed, State *state)
{
...
#ifdef MSG_DEBUG
  int size_of_vc = sizeof(View_change_rep) + sizeof(Req_info)
      * vc->rep().n_reqs;

  /*
   printf(
   "Replica %i, primary %i, view %qd, checking view change for view %qd (seqno=%qd) of size %i\n",
   replica->id(), replica->primary(), replica->view(), vi,
  ...
  /*
   printf(
   "BEFORE AUTHENTICATION: Replica %i, primary %i, view %qd, checking view change for view %qd (seqno=%qd) of size %i\n",
   replica->id(), replica->primary(), replica->view(), vi,
   vc->last_stable(), size_of_vc);
   // char *toto = vc->contents() + size_of_vc;
   //for (int i = 0; i < node->auth_size() / 4; i++)
   char *toto = vc->contents();
  ...
  for (int n = 0; n < nb_auth; n++)
  {
    ...
    /*
     fprintf(
     stderr,
     "AFTER AUTHENTICATION %i: Replica %i, primary %i, view %qd, checking view change for view %qd (seqno=%qd) of size %i\n",
     n, replica->id(), replica->primary(), replica->view(), vi,
     vc->last_stable(), size_of_vc);
     char *toto = vc->contents();
     for (int i = 0; i < vc->size() / 4; i++)
     {
  ...
  replica->send(vc, Node::All_replicas);
  ...
  if (primv != id)
  {
    ...
    for (int i = 0; i < node->n(); i++)
    {
      ...
      if (lvc && lvc->view() == v && i != id && i != primv)
      {
        ...
        /*fprintf(
         stderr,
         "Replica %i, primary %i, view %qd, sending a view change ack to %i for view %qd, acknowledging %i\n",
         replica->id(), replica->primary(), replica->view(), primv,
         vack->view(), vack->vc_id());*/
        replica->send(vack, primv);
      }
    }
  }
  else
...
bool View_info::add(View_change* vc)
{
  ...
  if (vcv < v)
  {
    ...
    fprintf(
     stderr,
     "\tReplica %i (primary=%i) (view %qd) handles a View change, adding View Change message failed because %i < %qd\n",
     node->id(), node->primary(), node->view(), vcv, v);
#endif
    return false;
  }
  ...
  if (n.view() == vcv)
  {
    ...
    else
    {
     ...
     fprintf(
     stderr,
     "\tReplica %i (primary=%i) (view %qd) handles a View change, adding View Change message failed because add returned false\n",
     node->id(), node->primary(), node->view());
     }

     fprintf(
     stderr,
     "\tReplica %i (primary=%i) (view %qd) handles a View change, id=%i, primv=%i, vcv=%i, v=%qd\n", node->id(), node->primary(), node->view(), id, primv, vcv, v);
#endif
    }
...
#ifdef MSG_DEBUG
       fprintf(stderr,
       "\tReplica %i (primary=%i) (view %qd) handles a View change, adding View Change message failed because %i <= %qd or !verified()\n",
       node->id(), node->primary(), node->view(), vcv, last_views[vci]);
#endif
    }
  }

#ifdef MSG_DEBUG
  fprintf(stderr, "\tReplica %i (primary=%i) (view %qd) handles a View change. stored=%s\n",
       node->id(), node->primary(), node->view(), (stored?"true":"false"));
#endif
