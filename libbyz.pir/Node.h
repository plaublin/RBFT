...
public:
  ...
  void sendTCP(Message *m, int i, int* sockets);
  // Send the message m to i using the TCP sockets in the array sockets

  void sendTCP_noblock(Message *m, int i, int* sockets);
  // Send the message m to i using the TCP sockets in the array sockets
  // non-blocking version

  void sendUDP(Message *m, int i);
  // Send the message m to i using UDP

  void sendUDP(Message *m, const Addr *to);
  // Send the message m to the addr *to using UDP

  void send_on_client_port(Message *m, int i, int port);
  ...
  bool
  verify_auth_out(int i, char *src, unsigned src_len, char *dest = 0) const;
  ...
  // if true, then a message is displayed in Node::verify_auth
  
  // protocol instance number of this PIR
  int protocol_instance_id;
...
protected:
 ...
 // A variable for sending messages to the different nodes in a round-robin manner
  int rr_client_last;
  ...
  // i.e., replicas_ports[i] = port on which the clients send requests to replica i
  int* replicas_ports;
...
inline int Node::n() const
{
  return num_replicas;
}

inline int Node::f() const
{
  return max_faulty;
}

inline int Node::n_f() const
{
  return threshold;
}

inline int Node::np() const
{
  return num_principals;
}

inline int Node::num_clients() const
{
  return num_principals - num_replicas;
}

inline int Node::num_pirs() const
{
  return max_faulty + 1;
}
...
inline Principal* Node::principal() const
{
  return i_to_p(id());
}
...
inline void Node::gen_auth_out(char *src, unsigned src_len, char *dest,
...
inline void Node::gen_auth_in(char *src, unsigned src_len, char *dest,
    bool faultyClient, bool to_print) const
{
  if (dest == 0)
    dest = src + src_len;
...
inline bool Node::verify_auth_in_print(int i, char *src, unsigned src_len,
    char *dest) const
{
  if (dest == 0)
    dest = src + src_len;
