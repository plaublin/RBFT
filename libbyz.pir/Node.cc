...
Node::Node(FILE *config_file, FILE *config_priv, short req_port)
{
  ...
  for (int i = 0; i < num_principals; i++)
  {
    ...
    bzero((char*) &a, sizeof(a));
    a.sin_addr.s_addr = inet_addr(addr_buff);
    a.sin_port = htons(port);     //PL: if you use UDP, then the port is not valid at this point because we don't know
                                  // yet the protocol_instance_id. The port is updated in the constructor of the PIR
  ...
  // set the protocol instance id to the value in the config files
  fscanf(config_file, "%d\n", &protocol_instance_id);
  
  fprintf(stderr, "Initializing a node for protocol instance %i\n",
      protocol_instance_id);
  ...
  //if the principal is a client, then bind on the port read ffom the configuration file.
  ...
  {
    //replica, bind on port 9876
    //    fprintf(stderr, "I am a replica, binding on port %d instead of port %d\n",principals[node_id]->get_main_thread_bind_port(),principals[node_id]->address()->sin_port);
...
void Node::sendTCP(Message *m, int i, int* sockets)
{
  ...
  {
    //  if(m->tag() == View_change_tag) fprintf(stderr, "node sending view change of size %d to all replicas\n",m->size());
    //fprintf(stderr, "\n Client is sending request to nodes ");
    for (int x = 0; x < num_replicas; x++)
    {
      //  if (x != id()) //some code relies on receiving self-created messages
      //fprintf(stderr, "%i",(rr_client_last + x) % num_replicas);
      sendTCP(m, (rr_client_last + x) % num_replicas, sockets);
    }
    rr_client_last = (rr_client_last + 1) % num_replicas;
...
void Node::sendTCP_noblock(Message *m, int i, int* sockets)
{
  ...
  if (i == All_replicas)
  {
    ...
    for (int x = 0; x < num_replicas; x++)
    {
      //  if (x != id()) //some code relies on receiving self-created messages
      //fprintf(stderr, "%i",(rr_client_last + x) % num_replicas);
      sendTCP_noblock(m, (rr_client_last + x) % num_replicas, sockets);
    }
    rr_client_last = (rr_client_last + 1) % num_replicas;
...
{
  /*
   fprintf(stderr, "Verifier %d sends a message of tag %d to %d.%d.%d.%d:%i\n",
