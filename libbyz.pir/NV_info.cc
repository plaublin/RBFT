...
NV_info::NV_info() :
  v(0), nv(0), vc_target(0), vc_cur(0), vcs(node->n())
{
...
bool NV_info::add(View_change *m, bool verified)
{
  ...
  if (vcs[vcid].vc != 0 || is_complete)
  {
    /*
     printf(
     "\t adding View Change message failed with vcid=%i and is_complete=%s\n",
     vcid, (is_complete ? "true" : "false"));
     */
    if (vcs[vcid].vc != 0)
    {
#ifdef MSG_DEBUG
      View_change* m2 = vcs[vcid].vc;
      /*
       fprintf(stderr, 
       "\t there is already a View_change: id=%i, view=%qd, last_stable=%qd, max_seqno=%qd\n",
       m2->id(), m2->view(), m2->last_stable(), m2->max_seqno());
       */
#endif
    }
    return false;
  ...
  if (!is_primary)
  {
    ...
    {
      /*
       fprintf(stderr, "\t adding View Change message failed with a=%s and b=%s\n",
  ...
  if (!verified)
  {
    if (is_primary || !m->verify_digest() || vcs[vcid].ack_count < node->f())
    {
      /*
       printf(
       "\t adding View Change message failed with is_primary=%s, vcs[%i].ack_count=%i and node->f()=%i\n",
       (is_primary ? "true" : "false"), vcid, vcs[vcid].ack_count, node->f());
       */
      return false;
    }
  }
...
bool NV_info::add(View_change_ack* m)
{
  ...
  if (!in_nv && m->vc_digest() == d && !vcs[vci].ack_reps.test(mid))
  {
    ...
    if (vcs[vci].ack_count == node->f() * 2 - 1 && node->primary(v)
        == node->id())
    {
...
#ifdef N_LE_GT_N_F_1
  if (comp_reqs[i - base] < 0 && cur.n_proofs >= node->f() + 1 && cur.n_le
          >= node->n_f() - 1)
#else
          //original code
  if (comp_reqs[i - base] < 0 && cur.n_proofs >= node->f() + 1 && cur.n_le
...
void NV_info::set_missing_pps(Status *m)
{
  for (Seqno i = base; i < max; i++)
  {
    ...
    {
      Req_sum& cur = reqsi[j];
      if (cur.v >= 0 && cur.v < vpp && cur.n_proofs >= node->f() + 1
          && cur.n_le >= node->n_f())
      {
        vpp = cur.v;
